...
double** insertionsort(double** arr, int column){
	...
	double** arr2 = (double**)malloc(sizeof(double*)*ROWS_INIT);
	...
	for (j=0; j<ROWS_INIT-1; j++){ //iterate through each row
...
int main(){
	...
	char*** farr_ins = converter2str_init(arr_ins);
	//export the 2d string array into a csv file
	export_csv_init(farr_ins, "./ocean_s.csv");
